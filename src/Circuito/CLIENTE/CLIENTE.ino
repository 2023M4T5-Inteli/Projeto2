...
const char* enderecoMac = "10.128.69.170"; // Endereço IP local do servidor

int portaServidor = 3002;// Porta usada pelo servidor;
...
String macs[qntMacs];//lista de endereços macs
  ...
  if (mensagem.length() == 12) {
    ...
    if (!encontrado) { //se for diferente de encontrado entra no "if"
      ...
      if (numMacs < qntMacs) { 
        ...
        client.println(macs[numMacs]);  // joga o para o serial do client o endereço mac que ele está conectado;

        numMacs++;//incrementa a variavel numMacs
